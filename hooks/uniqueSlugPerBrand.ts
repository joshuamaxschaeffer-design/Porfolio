 ...
 */
export const uniqueSlugPerBrand: Validate = async (value, { req, id, data, collection }) => {
  if (!value || typeof value !== 'string') return 'A slug is required.'
  if (!collection) return true // safety; should always be present on a collection field
  ...
  const result = await req.payload.find({
    collection: collection.slug as 'pages' | 'case-studies',
    where: {
      and: [
        { slug: { equals: value } },
