...
import { outcomes as defaults, type WingstopStat } from './data'

/**
 * Outcomes — fully built closing section. Same count-up engine as the Panda
 * Express OutcomesSection (per-cell IntersectionObserver, rAF ease-out-expo
 * 1.6s, 110ms row stagger, decimals support, reduced-motion snap) with
 * Wingstop-green accents. All figures are public reporting — sources in
 * data.ts + footnote.
 */
...
function StatCell({ stat, index }: { stat: WingstopStat; index: number }) {
  ...
  return (
    ...
    >
      ...
      >
        ...
        {stat.suffix ? <span className="text-[var(--ws-green)]">{stat.suffix}</span> : null}
      </p>
      <p className="br-data mt-3 text-[14px] uppercase tracking-[0.12em] text-[var(--ws-green)]">
        {stat.label}
      </p>
