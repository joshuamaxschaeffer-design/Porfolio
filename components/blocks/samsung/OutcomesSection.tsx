...
import { outcomes as defaults, type SamsungStat } from './data'

/**
 * Outcomes — fully built closing section. Same count-up engine as the Panda
 * and Wingstop OutcomesSections (per-cell IntersectionObserver, rAF
 * ease-out-expo 1.6s, 110ms row stagger, decimals support, reduced-motion
 * snap) with Samsung-blue accents. Era-scale figures, honestly framed —
 * sources in data.ts + footnote.
 */
...
function StatCell({ stat, index }: { stat: SamsungStat; index: number }) {
  ...
  return (
    ...
    >
      ...
      >
        ...
        {stat.suffix ? <span className="text-[var(--sg-blue)]">{stat.suffix}</span> : null}
      </p>
      <p className="br-data mt-3 text-[14px] uppercase tracking-[0.12em] text-[var(--sg-blue)]">
        {stat.label}
      </p>
