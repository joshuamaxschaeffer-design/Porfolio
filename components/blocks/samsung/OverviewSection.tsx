...
/** Section 1 — wordmark, date, lead, role, scope pills (Samsung blue accent). */
export function OverviewSection(props: OverviewProps) {
  ...
  return (
    <section id="overview" className="br-container pt-16 pb-20 md:pt-24 md:pb-[160px]">
      ...
      <p
        className="text-[28px] font-semibold uppercase leading-none tracking-[0.02em] text-[var(--sg-blue)] md:text-[34px]"
      ...
      <div className="mt-7">
        ...
        <ul className="mt-2.5 flex flex-wrap gap-2">
          {scope.map((s) => (
            <li
              key={s}
              className="br-data rounded-[var(--br-tag-radius)] border border-[var(--sg-blue)] px-3 py-1.5 text-[14px] uppercase text-[var(--sg-blue)]"
            >
              {s}
            </li>
          ))}
