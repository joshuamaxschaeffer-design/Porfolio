...
export function ChallengeSection(props: ChallengeProps) {
  ...
  const onPointerDown = (e: React.PointerEvent) => {
    const el = trackRef.current
    if (!el) return
    // TOUCH scrolls natively (momentum + snap) — JS drag is MOUSE-ONLY.
  ...
  return (
    <section id="challenge" className="bg-[var(--br-bg-2)] py-20 md:py-[160px]">
      ...
      {/* Track at ALL sizes, bleeds to both edges.
          MOBILE/TOUCH: fully NATIVE scrolling — momentum + mandatory snap so
          each swipe lands cleanly on a card. JS never intercepts touch (the
          old rAF scrollLeft drag is what made it feel like it "didn't cleanly
          scroll" under a finger). touch-action allows both axes natively.
          DESKTOP (mouse, lg+): JS 1:1 drag + flick momentum, snap off. No
          scroll-smooth — CSS smooth animates every programmatic scrollLeft
          write and fights the rAF loops.
          (EdgeFadeBlur's band is max(0,(100vw-1443px)/2) = 0 on phones, so the
          fade/blur is inert there — it never affected mobile.) */}
      <EdgeFadeBlur bg="var(--br-bg-2)" className="mt-8">
        <div
          ref={trackRef}
          className="br-noscrollbar br-grab flex snap-x snap-mandatory gap-5 overflow-x-auto overscroll-x-contain scroll-pl-6 pb-2 pl-6 pr-6 select-none md:scroll-pl-[calc(max(0px,(100vw-1443px)/2)+5rem)] md:pl-[calc(max(0px,(100vw-1443px)/2)+5rem)] lg:snap-none"
...
function ChallengeCardView({
  ...
  return (
    <article
      ...
      className={`relative ${card.mobileImage ? `${MOBILE_CARD_RATIO} lg:aspect-[1920/1150]` : CARD_RATIO[card.span]} w-[88vw] max-w-[88vw] shrink-0 snap-start rounded-2xl bg-white lg:h-[600px] lg:w-auto lg:max-w-none lg:snap-align-none`}
    >
