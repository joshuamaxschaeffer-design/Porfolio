'use client'

import { useEffect } from 'react'

/**
 * Error boundary for the frontend. Catches unexpected runtime errors
 * in any route and shows a graceful recovery UI.
 */
export default function Error({
  error,
  reset,
}: {
  error: Error & { digest?: string }
  reset: () => void
}) {
  useEffect(() => {
    // TODO: pipe to Sentry / your error tracker once configured
    console.error(error)
  }, [error])

  return (
    <div className="container flex min-h-[60vh] flex-col items-center justify-center px-6 text-center">
      <p className="font-mono text-sm uppercase tracking-wider text-neutral-500">Something broke</p>
      <h1 className="mt-3 text-3xl font-semibold tracking-tight md:text-4xl">
        Sorry — that didn't load.
      </h1>
      <button
        onClick={() => reset()}
        className="mt-8 inline-flex items-center gap-2 rounded-full border border-neutral-900 px-6 py-3 text-sm font-medium transition-colors hover:bg-neutral-900 hover:text-white dark:border-neutral-100 dark:hover:bg-neutral-100 dark:hover:text-neutral-900"
      >
        Try again
      </button>
    </div>
  )
}
